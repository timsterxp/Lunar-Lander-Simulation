...
void ofApp::setup() {
	...
	bgMusic.load("sound/bensound-slowmotion.mp3", true);
	bgMusic.setLoop(true);
	bgMusic.play();
	bgMusic.setVolume(0.4f);
	thrustSound.load("sound/Rocket.wav", false);
	thrustSound.setVolume(1.0f);

	
	font.load("seaside.ttf", 50, true, true, true);
	font2.load("seaside.ttf", 20, true, true, true);
	font3.load("seaside.ttf", 20, true, true, true);
	ofDisableArbTex();
	...
	bBackgroundLoaded = backgroundImage.load("images/starfield-purple2.jpg");
...
void ofApp::draw() {
	...
	if (crashed) {
			ofBackground(ofColor::black);
		ofPushMatrix();
		string titleText = "You were moving too fast and crashed!\n To restart press the space bar.";
		ofTranslate(-70, ofGetWindowHeight()/4);
		font.drawString(titleText, font.stringWidth(titleText)/15, 0);
		ofPopMatrix();
		return;
	...
	ofDrawBitmapString(goals, 5, 60);
	string modifiers;
	modifiers += "To increase your score you can: \n - Reach an altitude >100 \n - Have 0 collisions \n - Reduce time to landing";
	ofDrawBitmapString(modifiers, 5, 75);
	...
	string start;
	if (!bLanderLoaded) {
		start = "To start, drag an .obj file into the viewpane \n or click anywhere to spawn the default";
	...
	if (bLanderLoaded) {
			...
			str3 += "Use the directional and W/S keys to move around the map";
			ofSetColor(ofColor::white);
			str4 += "Use the F1-F6 keys to change camera views";
	...
	if (gameFinished) {
		...
		else if (startTime < 100 && startTime>50) {
			totalScore = 100;
		}
		...
		string finishText = "Completed! Your score is: " + std::to_string(totalScore);			
